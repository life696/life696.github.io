...
- 概率：由于事件A发生的频率$f_n(A)$随着试验次数的增加稳定于概率$P(A)$，因此可以用频率$f_n(A)$来估计概率$P(A)$,称为事件A的概率。概率$P(A)$频率趋近于某一个固定常数，在$[0,1]$之间
...
古典概型的概率计算公式为$P(A)=\frac{A所包含的基本事件的个数}{总的基本事件个数}$

### 几何概型

1.几何概率模型：如果每个事件发生的概率只与构成该事件区域的长度（面积或体积）成比例，则称这样的概率模型为几何概率模型。几何概型的概率公式：$P(A)=\frac{构成事件A的区域长度(面积、体积)}{实验的全部结果所构成的区域长度(面积、体积)}$，具有以下特点：
...
![8878b7d3e91224c99a44ebd812c17d2](高中数学_笔记_img/8878b7d3e91224c99a44ebd812c17d2.png)
