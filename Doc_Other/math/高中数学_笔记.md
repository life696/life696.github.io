...
2.象限角：平面直角坐标系被x,y轴分成四个部分，四个象限，第一象限、第二象限、第三象限、第四象限，角的终边在哪个象限，就叫哪个象限的角
...
- 三角函数在各个象限的正负
  ...
  <img src="C:/Users/zhang/Desktop/数学/高中数学_笔记_img/image-20210507144158019.png" alt="image-20210507144158019" style="zoom:33%;" />
...
2.直线的斜率：把一条直线的倾斜角$\alpha$的正切值叫做这条直线的斜率，即$k= \tan \alpha$。若经过两点$A(x_1,y_1),B(x_2,y_2)$的直线的斜率的公式为$k=\frac{y_2-y_1}{x_2-x_1}$或$k=\frac{y_1-y_2}{x_1-x_2}$,所以有如下结论：

- 当$ \alpha = 0$时，$ k = 0$,直线平行于x轴
- 当$0^\circ <  \alpha < 90^\circ$时，$ k > 0$
- 当$ \alpha = 90^\circ$时，$ k$不存在
...
- 关于x轴对称：$Ax-By+C=0$
- 关于y轴对称：$-Ax+By+C=0$或$Ax-By-C=0$
- 关于原点对称：$-Ax-By+C=0$或$Ax+By-C=0$

### 圆与方程

1.圆的方程

- 圆的标准方程：$(x-a)^2+(y-b)^2=R^2$，其中$(a,b)$为圆心的坐标，$R$为半径。当圆心为原点时，圆的方程为$x^2+y^2=R^2$
...
- 相交：直线与圆有两个交点$\Leftrightarrow$方程组解有两组$\Leftrightarrow$圆心到直线距离小于半径
  ...
  <img src="C:/Users/zhang/Desktop/数学/高中数学_笔记_img/image-20210506141152266.png" alt="image-20210506141152266" style="zoom:33%;" />

3.圆与圆的位置关系：外离、外切、相交、内切、内含、同心圆，设圆心为$O_1、O_2$，半径为$R_1,R_2$($R_1>R_2$)有：

- 外离：圆心距离大于半径之和，无交点，即$\vert O_1O_2\vert> R_1+R_2$

- 外切：圆心距离等于半径之和，仅一个交点，即$\vert O_1O_2\vert = R_1+R_2$

- 相交：圆心距离小于半径之和，有两个交点，即$\vert O_1O_2\vert < R_1+R_2$

- 内切：圆心距离等于半径之差，仅一个交点，即$\vert O_1O_2\vert> R_1-R_2$

- 内含：圆心距离小于半径之差，即$\vert O_1O_2\vert> R_1-R_2$

- 同心圆：圆心距离为0，即$\vert O_1O_2\vert = 0$

  ![image-20210506143305651](高中数学_笔记_img/image-20210506143305651.png)
...
- 若m+n=p+q,则$a_ma_n=a_pa_q$,m,n,p,q均为正整数。特别地，m+n=2p时，$a_ma_n=a_p^2$
...
- $\frac{2ab}{a+b} \leqslant \sqrt{ab} \leqslant \frac{a+b}{2} \leqslant \sqrt{\frac{a^2+b^2}{2}},a,b>0$
...
- 随机事件：频数为$n_A$,频率为$f_n(A)=\frac{n_A}{n}$,在$[0,1]$之间
- 概率$P(A)$频率趋近于某一个固定常数$[0,1]$
...
计算公式为$P(A)=\frac{A所包含的基本事件的个数}{}=\frac{m}{n}$
