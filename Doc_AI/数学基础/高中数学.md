...
**3.**数量积的几何意义：$\vert\vec{b}\vert \cos \theta$叫做向量$\vec{a}$在$\vec{b}$方向上($\vec{b}$在$\vec{a}$方向上)的**投影**(结果是一个数)。$\vert\vec{b}\vert \cos \theta   = \frac{\vec{a} \cdot  \vec{b} }{\vert\vec{a}\vert} $
...
- $ \cos \theta  = \frac{\vec{a} \cdot  \vec{b} }{{\vert\vec{a}\vert}{\vert\vec{b}\vert}}$      （解决角度问题）
