...
1.向量夹角的定义：已知两个非零向量$\vec{a}$和$\vec{b}$,作$\vec{OA} = \vec{a}$,$\vec{OB} = \vec{b}$，则$\angle AOB=\theta(0^\circ \leqslant \theta \leqslant 180^\circ )$叫做向量$\vec{a}$与$\vec{b}$的夹角。
...
**5.**数量积性质：设$\vec{a}$与$\vec{b}$为非零向量，$\theta$是$\vec{a}$与$\vec{b}$的夹角，则

- $\vec{a} \cdot  \vec{b} =0$ 等价于 $\vec{a}\perp\vec{b}$ （解决垂直问题）
- 当$\vec{a}$与$\vec{b}$同向时，$\vec{a} \cdot  \vec{b}=\vert \vec{a}\vert\vert\vec{b}\vert $；当当$\vec{a}$与$\vec{b}$反向时，$\vec{a} \cdot  \vec{b}=-\vert \vec{a}\vert\vert\vec{b}\vert $   （解决长度问题）
- $\vec{a} \cdot  \vec{a} = \vec{a}^2=  \vert\vec{a} \vert^2$ 等价于 $\vert\vec{a} \vert = \sqrt{\vec{a} \cdot  \vec{a} }   $
- $ \cos \theta  = \frac{\pmb{a} \cdot \pmb{b} }{\vert \pmb{a}\vert \vert \pmb{b}\vert}$      （解决角度问题）
- $-\vert\vec{a}\vert \vert\vec{b}\vert  \leqslant \vec{a} \cdot  \vec{b} \leqslant \vert\vec{a}\vert \vert\vec{b}\vert $,当$\vec{a}$与$\vec{b}$共线时取“=”。其中，当$\vec{a}$与$\vec{b}$共线且同向时，$\vec{a} \cdot  \vec{b}=\vert \vec{a}\vert\vert\vec{b}\vert $；当$\vec{a}$与$\vec{b}$共线且反向时，$\vec{a} \cdot  \vec{b}=-\vert \vec{a}\vert\vert\vec{b}\vert  $   （解决数量积的范围和最值问题）
...
- 若$\pmb{a} \perp\pmb{b}$，则$\vec{a} \cdot  \vec{b} =\vert \vec{a}\vert\vert\vec{b}\vert \cos 0^\circ = x_1x_2+y_1y_2=0$
