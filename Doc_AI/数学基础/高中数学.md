...
**1.**向量：既有大小，又有方向的量。向量的大小叫向量的长度（或模），使用两个|夹住。例如向量$ \vec{a}$的长度/模为,$\left | \vec{a} \right |$向量$\vec{AB}$的长度/模为$\left | \vec{AB} \right |$

**2.**零向量：长度为零的向量，方向可以使任意的。记作$\vec{0}$,可以理解为一个点。零向量平行(共线)于任一向量

**3.**单位向量：长度为1个单位的向量，非零向量$\vec{a}$的单位向量为$\pm \frac{\vec{a}}{|\vec{a}|}$,单位向量不一定是相等的

**4.**平行向量：方向相同或相反的非零向量

**5.**共线向量：方向相同或相反的非零向量又叫共线向量，即$\vec{a} // \vec{b}$。平行向量也成为共线向量，两个概念之间没有区别
...
- $\Delta ABC$中：
  ...
  - 重心：中线的交点，重心到顶点的距离是到相应边上中点距离的两倍，即：$\vec{PG} = \frac{1}{3}(\vec{PA}+\vec{PB}+\vec{PC})$$\Leftrightarrow$G为重心。当$\vec{PA}+\vec{PB}+\vec{PC} = 0$$\Leftrightarrow$P为重心
  - $\vec{PA}=\vec{PB}=\vec{PC}$$\Leftrightarrow$P为外心
- 由向量加法、减法的集合意义，可得向量不等式：$\left |\left | \vec{a} \right  | - \left | \vec{b} \right   | \right |\leqslant \left | \vec{a} \pm \vec{b}  \right | \leqslant \left | \vec{a} \right | + \left | \vec{b} \right |$
...
- $\vec{a} // \vec{b}\Leftrightarrow \vec{b}=\lambda \vec{a}($$\vec{a} \neq \vec{0}$)，是判断两个向量共线的主要依据；证明三点共线问题，可用向量共线解决，但是要注意向量共线与三点共线的区别联系当两向量共线且有公共点时，才能得出三点共线

- 三点共线的等价关系：A、P、B三点共线$\Leftrightarrow$$ \vec{AP}$
